...
from .clip_func import CLIPFunction
import torch
...
class VLMGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # model.visual.blocks = model.visual.blocks[:20]
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        # 从输入数据中提取 image_path 和 normal_caption
        image_paths = [example["image_path"] for example in inputs]
        normal_captions = [example["normal_caption"] for example in inputs]
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            # if isinstance(reward_func, CLIPFunction):
            if isinstance(reward_func, nn.Module):
                ...
                with torch.inference_mode():
                    output_reward_func = reward_func(completions, image_paths=image_paths, normal_captions=normal_captions)
                    # print('rewards_per_func_0:', rewards_per_func)
                    # print('output_reward_func_1:', output_reward_func)
                    rewards_per_func[:, i] = torch.tensor(output_reward_func, dtype=torch.float32, device=device)
                    # print('rewards_per_func_1:', rewards_per_func)
            else:
                # Repeat all input columns (but "prompt" and "completion") to match the number of generations
                reward_kwargs = {
                    # "image_paths": image_paths,  # 新增参数
                    # "normal_captions": normal_captions,  # 新增参数
                ...
                # reward_kwargs['image_paths'] = image_paths
                # reward_kwargs['normal_captions'] = normal_captions
                output_reward_func = reward_func(prompts=prompts, completions=completions, **reward_kwargs)
                # print('output_reward_func_2:', output_reward_func)

                rewards_per_func[:, i] = torch.tensor(output_reward_func, dtype=torch.float32, device=device)
                # print('rewards_per_func_2:', rewards_per_func)
