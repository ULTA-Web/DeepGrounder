...
class Qwen2VLGRPOVLLMTrainer(Trainer):
    def __init__(
        ...
        self._metrics["cot_length"] = []  # 初始化COT长度指标
    ...
    def _prepare_inputs(
        ...
        # 计算COT长度
        cot_lengths = [len(completion.split()) for completion in completions]
        average_cot_length = sum(cot_lengths) / len(cot_lengths)
        self._metrics["cot_length"].append(average_cot_length)
