...
# 获取 open_r1 模块所在的目录
open_r1_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
# 将该目录添加到 Python 模块搜索路径
sys.path.append(open_r1_dir)
...
class GRPOScriptArguments(ScriptArguments):
    ...
    reward_funcs: list[str] = field(
        default_factory=lambda: ["accuracy", "format", "semantic"],
...
def main(script_args, training_args, model_args):
    ...
    #
    # # 计算加权奖励函数
    # def weighted_rewards(completions, solution, **kwargs):
    #     image_paths = kwargs.get('image_path')
    #     normal_captions = kwargs.get('normal_caption')  # 获取数据集中的 normal_caption
    #     return vlm_module_cls.weighted_rewards(completions, solution, image_paths, normal_captions, **kwargs)
    #
    # # 替换原来的奖励函数列表为加权奖励函数
